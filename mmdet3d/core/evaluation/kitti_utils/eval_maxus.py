...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            gt_boxes = np.concatenate([a['box3d_camera'][:, [0, 2, 3, 5, 6]] for a in gt_annos_part], 0)
            dt_boxes = np.concatenate([a['box3d_camera'][:, [0, 2, 3, 5, 6]] for a in dt_annos_part], 0)
            overlap_part = bev_box_overlap(gt_boxes,
                                           dt_boxes).astype(np.float64)
        elif metric == 2:
            gt_boxes = np.concatenate([a['box3d_camera'] for a in gt_annos_part], 0)
            dt_boxes = np.concatenate([a['box3d_camera'] for a in dt_annos_part], 0)
